...
data = "John said that it is a very small world."
#Needed in order to keep track of sentences 
nlp = stanfordnlp.Pipeline() # This sets up a default neural pipeline in English
doc = nlp(data)

response = requests.post("http://localhost:7000/v1/parsey-universal-full", headers=headers, data=data)
z = response.text
print(z)

#Write results from parser to file
i = 0
f = open('syntaxParse.txt', 'a')
f.truncate(0)
f.write(z)
f.close()

#Read lines with amod rship
pairArray = []
j = open('syntaxParse.txt', 'r')
for line in j:
    if 'amod' in line:
        x = line.split() 
        pairArray.append(x)
